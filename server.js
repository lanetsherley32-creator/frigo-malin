...
app.get('/api/courses', async (req, res) => {
   const { profil, enseigne } = req.query;
   if (!profil) return res.status(400).json({ error: "Profil manquant" });
   try {
       ...
       console.error("Erreur GET /api/courses :", err.message);
       res.status(500).json({ error: err.message });
   }
});
...
async function ajouterIngredientDansCourses(profil, nomIng, qte, unite) {
   ...
   await pool.query(
       ...
       [
           ...
           prixRetenu,                   
           marqueRetenue,                
           JSON.JSON.stringify(marquesDispos), // Corrigé ici si nécessaire ou conservé selon la source
           0                             
       ]
   );
}
